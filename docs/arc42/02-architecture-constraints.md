...
|----|-----------|--------|-------------|
| TC-1 | **Python 3.12+ standard library only** for the core pipeline (`scripts/`) | Reproducibility goal (Q2), see [ADR-001](09-architecture-decisions.md#adr-001--zero-external-python-dependencies) | No `pip`, no `venv`, no `requirements.txt` for ingest / query. HTTP via `urllib.request`; JSON via `json`; XML via `xml.etree.ElementTree`; search via `sqlite3` with FTS5; concurrency via `concurrent.futures`. **Scoped exceptions** for the optional surfaces in `web/`: `fastapi` (web UI server) and `ddgs` (opt-in web-search augmentation). These deps are never reached by the offline-only code path in `scripts/`; `pyproject.toml` declares them as runtime extras. |
...
| TC-5 | **Gemma 4 reasoning mode is a toggle, and ingestion requires `off`** | Observed failure, see [Appendix A, Failure F-3](appendix-a-academic-retrospective.md#f-3--thinking-tokens-consume-the-output-budget) | `scripts/start_server.sh` exposes a single `REASONING` variable. Default is `on` because Gemma 4's `<think>` phase noticeably improves conversational answer quality. For **ingestion** the value must be `off`: invisible thinking tokens otherwise consume the output budget and the entity-extraction JSON truncates to an empty object. The toggle is also wired into the web UI header so operators can flip it per run. The [Gemma 4 model card](https://ai.google.dev/gemma/docs/core/model_card_4) documents that non-E2B/E4B variants skip the thinking phase entirely when the flag is set. |
...
| TC-9 | **PDF parsing delegates to `pdftotext`** (Poppler) via `subprocess.run` with the list form | No `PyPDF2`/`pdfminer` allowed under TC-1 | `subprocess.run` uses `shell=False` (the list form) with no shell interpolation, so filename metacharacters cannot reach a shell. |

## 2.2 Organizational Constraints

| ID | Constraint | Rationale |
|----|-----------|-----------|
| OC-1 | **Single-operator, single-machine deployment** | This is a personal-knowledge-base POC. There is no multi-tenant story, no auth system, no user management, no RBAC. The threat model is explicitly single-user. |
...
| OC-4 | **No secrets in git history** (past or future) | Enforced by `.gitignore` (see [section 7.4](07-deployment-view.md#74-repository-hygiene-and-rebuildable-state)); the repo declares `dependencies = []` so there are no credentialled package registries in play. |
...
- **OCR of scanned PDFs.** `pdftotext` handles text PDFs; image-only PDFs produce empty extraction and abort. See [section 11.1, limitation L-7](11-risks-and-technical-debt.md#l-7--ocr-of-scanned-pdfs-is-not-supported).
- **A remotely accessible web UI.** The optional FastAPI app under `web/` binds to `127.0.0.1:3000` by default, ships no authentication and no rate limiting, and is explicitly a localhost-only single-operator surface. Exposing it over a LAN or the public internet is out of scope and would require an authenticating reverse proxy and rate limits added outside the app. See the [README "Security and privacy posture"](../../README.md#security-and-privacy-posture) callout.
