...
> **Companion to the arc42 main body.** The 12 arc42 sections describe *what the system is today*. This appendix documents *how it got there*, the design trade-offs, the components kept versus gated versus dropped, and the labelled failure modes encountered during development. Three categories: (A) components that worked and fit the pipeline, (B) components that succeeded but did not fit the project's purpose, and (C) failure modes catalogued with symptom, root cause and resolution.
>
> If the main body is the *architecture*, this appendix is the *reasoning*.
...
Earlier versions had three subtly-different implementations of "strip path separators and make a filename safe" scattered across `ingest.py`, `query.py` and `cleanup_dedup.py`. One version failed to normalise Unicode whitespace, producing mismatched paths on non-ASCII input. The fix was consolidation into one `llm_client.safe_filename()` with a single test file. Similarly, `find_existing_page()` was duplicated in two places; both were replaced with a single implementation that consults the SQLite reverse index.
...
3. On a representative mixed-language corpus, Stage 0 (gazetteer) + Stages 3-4 (Jaccard + judge) already resolve > 95 % of mentions correctly. The marginal benefit of Stage 5 is < 5 % of mentions, and most of those are cases where adding an entry to the gazetteer is a cleaner fix.

**Current status.** Implemented, tested, opt-in via `--use-embeddings`. Left in the tree because it is sound, because it is the only Stage-5 option that handles cross-lingual mentions, and because on a different corpus the cost-benefit might flip.
...
**Why it did not fit the default.** It composes with Stage 5 (embedding cosine) in a way that depends on the calibration data, and typical POC-scale corpora do not have enough > 10-year spans to make the feature measurably useful. It is implemented in `resolver.py` but gated behind `--use-embeddings`, so it activates only on opt-in for longer-running corpora.
...
**Current status.** The stemming function is archived in a branch. If a future corpus tilts more heavily toward Greek, it could be resurrected with a pre-stemming approach for Greek-only pages.
...
**Why it worked.** The community detection was correct on a test wiki of 200 pages. Communities corresponded roughly to recognisable topic clusters.

**Why it did not fit the default.** Two reasons:

1. The community summaries duplicated information already in the `wiki/concepts/` and `wiki/synthesis/` pages, which are human-readable and already cross-linked. Adding another layer of auto-summaries did not improve query answers in manual testing.
...
**Root cause.** F1 is not robust on heavily imbalanced data. With 51 positives and 1 negative, the "optimal" threshold is whatever accepts all positives, regardless of how tight the margin is. The tuner was behaving as specified; the data was pathological.
...
**Lesson.** Auto-tuning on cached runtime data is dangerous when the cache has a natural bias. The resolver's cache has a bias because easy positives accumulate faster than hard negatives (curators label "yes, these are the same" more often than "no, these are different" when using the system). The gates made the tuner robust to this bias.
...
**Lesson.** Read the model card, including the footnotes, before the first prompt. Also: when HTTP returns 200 but the content is empty, the problem is always on the model side, sampling, token budget, or template, never on the client. Inspecting the server's debug logs before grep-ing for HTTP errors reduces the root-cause time from hours to minutes.
...
- **When:** After ingesting ~ 50 sources during resolver v2 development
...
- **When:** Resolver v2 testing on a biology paper
- **Severity:** Medium, one false fork per re-ingest

**Symptom.** Ingesting a paper about the Zika virus produced a `wiki/entities/Aedes aegypti.md` entity page with `type: organism`. Re-ingesting the same paper during idempotency testing produced a second page, `wiki/entities/Aedes aegypti (model).md`. The two pages described the same mosquito species.
...
**Mitigation.** Narrowed Stage 2 from "fork on type mismatch" to "fork on type mismatch *AND* low Jaccard". The rationale is that a type change alone is not enough evidence; if the descriptions match, the type change is probably LLM noise, not a real polysem. The narrowed rule is in `resolver._stage_2_type_constraint()`.
...
D-5 and [ADR-002](09-architecture-decisions.md#adr-002--fork-on-uncertainty-never-silently-merge) both taught this. A personal knowledge base that answers differently for the same question over time is untrustworthy. Silent merges of entities, silent compressions of pages, silent rewrites of any kind, all produce the same symptom: the user can't verify that the wiki means what it used to mean. The antidote is to fork on uncertainty, preserve rich content and expose all decisions in the resolver's `reason` field so a human can review them.
...
F-5 taught this. The `turbo3` setting is safe on most open-weights models; it is catastrophic on Gemma 4 Q4_K_M. Trusting a global recommendation would have produced garbage output before the regression surfaced. The fix is to run the community benchmark *on your actual target model* before committing. This sounds obvious and is often skipped.

---

## A.6 What Would Be Different Next Time

Not every choice would be repeated with the benefit of hindsight. Specific revisions for a hypothetical rebuild:

- **The first resolver should have been the six-stage resolver, not the three-stage one.** The entity-linking literature (BLINK, ReFinED, TAGME, mGENRE) was well-understood at design time. The four-stage resolver was built under a "start simple and iterate" heuristic, which is usually right but was not here, the literature already indicated that a gazetteer-first design wins for known entities. Iterating from v1 to v3 cost development time that could have been spent elsewhere.
- **The `--reasoning off` flag should have been validated on Gemma 4 before any prompt work.** The flag is documented in the Gemma 4 model card; a model-card read at project start would have avoided the empty-output debugging loop.
- **A comprehensive `.gitignore` should have been in the first commit.** A minimal initial ignore file was briefly in place before the full exclusion set was added. Starting with the aggressive `.gitignore` from commit one would have been the right move.
- **The judge cache should have been atomic-write from the start.** It uses `json.dump` directly and could be corrupted by a crash during a write. The cache rebuilds, so this is never a data-loss scenario, but it is a hygiene issue (tracked as [TD-8](11-risks-and-technical-debt.md#112-technical-debt)).
- **Tests should have been written alongside the resolver, not after the failures.** `test_resolver_scenarios.py` was written after F-4 and F-6 had already shipped. Writing it first would have caught both earlier. TDD discipline is most valuable on exactly the modules where it is most tempting to skip.

---

## A.7 Closing Note

A personal knowledge base is a long-term artefact. The code that produces it today will be superseded by something better in a year. The retrospective in this appendix is an attempt to carry forward what was learned, so the next iteration, whether a reimplementation in this repository or an independent fork, does not re-learn the same lessons.
